...
class _GameScreenState extends State<GameScreen> with TickerProviderStateMixin {
  ...
  bool get isDuckMode => widget.birdType.name == 'Duck';
  ...
  void _startLevel() {
    ...
    // Spawn 30 birds evenly over 30 seconds
    final spawnInterval = (levelTime * 1000 / birdsPerLevel).round();
    spawnTimer?.cancel();
  ...
  void _spawnBird() {
    ...
    // For Random Birds mode: 
    // 0=custom drawn bird, 1=duck emoji, 2=goose emoji, 
    // 3=swan, 4=coot, 5=moorhen, 6=heron
    int birdStyle = 0;
    if (!isDuckMode) {
      birdStyle = random.nextInt(7); // Randomly pick one of 7 types
    }
  ...
  void _shoot(Offset position) {
    ...
    for (var bird in birds) {
      ...
      final birdCenter = Offset(bird.x + 35, bird.y + 25);
      final distance = (position - birdCenter).distance;
      const hitRadius = 55.0;
      ...
      if (distance < hitRadius) {
        ...
        int points = widget.birdType.points;
        String hitType = 'good';
        ...
        child: Stack(
          children: [
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: const RadialGradient(
                    colors: [Color(0xFFFFFDE7), Color(0xFFFFF59D), Color(0xFFFDD835)],
                  ),
                  boxShadow: [
                    BoxShadow(
              ...
              child: Container(
                ...
                decoration: const BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [Colors.transparent, Color(0xFF0a1208)],
                  ),
                ),
              ),
            ),
            ...
            Positioned(
              ...
              left: 0,
              right: 0,
              child: Container(height: 40, color: const Color(0xFF081a08)),
            ),
  ...
  Widget _buildBird(Bird bird) {
    if (isDuckMode) {
      // Duck mode - just duck emoji, no animation
      return Positioned(
        left: bird.x,
        top: bird.y,
        child: Opacity(
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text('HUNT COMPLETE!', style: TextStyle(color: Colors.amber, fontSize: 28, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Text('$score', style: const TextStyle(color: Colors.amber, fontSize: 50, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Text('$hits hits • ${shots > 0 ? ((hits / shots) * 100).round() : 0}% accuracy',
...
class Bird {
  ...
  int deadTime;
  Color color;
  int birdStyle; // 0=custom drawn, 1=duck emoji, 2=goose emoji
